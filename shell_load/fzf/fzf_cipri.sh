...
# so - fuzzy search and open (with default application)
so() {
  local res
  IFS=$'\n' res=($(fzf-tmux --query="$1" --multi --select-1 --exit-0))
  [[ -n "$res" ]] && open "${res[@]}"
}

# sv - fuzzy search and open with vim
sv() {
  local files
